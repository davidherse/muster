...
CONTEXT.each do |name, extra|
  doc = ACCOUNT.training_documents.find_by!(name: name)
  if ENV["FORCE"].blank? && doc.status == "completed" &&
     PriceBookItem.from_training_doc(ACCOUNT, doc.id).any? { |i| i.context.to_h["qty_kind"].present? }
  ...
  with_qty = PriceBookItem.from_training_doc(ACCOUNT, doc.id).count { |i| i.context.to_h["qty_kind"] == "measured" }
  puts "  -> #{doc.reload.status}, #{PriceBookItem.from_training_doc(ACCOUNT, doc.id).count} entries (#{with_qty} measured)"
end

puts JSON.pretty_generate(ACCOUNT.reload.quantity_norms)
